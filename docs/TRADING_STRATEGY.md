...
| 上昇閾値 | 0.3% | ラベル1の基準 |
| 確信度閾値 (LONG) | **75%** | LONG通知の最低確信度 |
| 確信度閾値 (SHORT) | **80%** | SHORT通知の最低確信度（より厳格） |

### 9.2 リスクパラメータ

| パラメータ | LONG | SHORT | 説明 |
|-----------|------|-------|------|
| 1トレードリスク | 2% | 1.5% | 資金に対する最大損失 |
| 最大ポジション | 10% | 7% | 資金に対する最大ポジション |
| ストップロス | 2×ATR | 1.5×ATR | ATRベースの損切り |
| 最大日次取引 | **2回** | **1回** | 方向別の最大取引回数 |
| 時間ストップ | 4時間 | 4時間 | 最大保有時間 |
...
| 週次損失上限 | 7% | 当週取引停止ライン |
| 月次損失上限 | 12% | 当月取引停止ライン |
| 日次取引上限 | **3回** | 1日の最大取引回数（税金効率化のため削減） |
...
| パラメータ | 値 | 説明 |
|-----------|-----|------|
| TP1 | **2.0R, 33%** | 2倍リスクで1/3決済 |
| TP2 | **3.0R, 33%** | 3倍リスクで1/3決済 |
| TP3 | **5.0R, 34%** | 5倍リスクで残り決済 |

**SHORT（早めに利確）:**
| パラメータ | 値 | 説明 |
|-----------|-----|------|
| TP1 | **1.5R, 40%** | 1.5倍リスクで40%決済 |
| TP2 | **2.0R, 35%** | 2倍リスクで35%決済 |
| TP3 | **3.0R, 25%** | 3倍リスクで残り決済 |

### 9.4 フィルターパラメータ

| パラメータ | LONG | SHORT | 説明 |
|-----------|------|-------|------|
| 最小確信度 | **75%** | **80%** | シグナル通知の閾値 |
| 最大スプレッド | 5bps | 5bps | 取引可能スプレッド上限 |
| 最小出来高比 | 80% | 80% | 20MA比での最小出来高 |
| シグナル有効期限 | 15分 | 15分 | 承認待ちタイムアウト |
...
|---------|------|------|
| ~195万円 | 15% | 所得税5% + 住民税10% |
| ~330万円 | 20% | 所得税10% + 住民税10% |
| ~695万円 | 30% | 所得税20% + 住民税10% |
| ~900万円 | 33% | 所得税23% + 住民税10% |
| ~1,800万円 | 43% | 所得税33% + 住民税10% |
| ~4,000万円 | 50% | 所得税40% + 住民税10% |
...
1. **取引頻度の制限**: 日次最大3回（税金コスト削減）
2. **高確信度のみ取引**: 勝率向上で純利益最大化
3. **損出し機能**: 年末の含み損決済で課税所得削減
4. **税引後レポート**: バックテスト結果に税引後リターンを表示
